...
export default function RegisterPage() {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-4 py-28">
      <div className="fixed top-1/4 right-1/4 w-80 h-80 rounded-full blur-[150px] opacity-[0.06]" style={{ background: "#ec4899" }} />
      <div className="fixed bottom-1/4 left-1/4 w-64 h-64 rounded-full blur-[120px] opacity-[0.05]" style={{ background: "#facc15" }} />
      ...
      <div className="w-full max-w-md relative">
        ...
        >
          <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-pink-400/40 to-transparent rounded-t-3xl" />
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-white/35">Join 10K+ developers on Codevault</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {success ? (
              <div className="text-center space-y-4">
                ...
                <p className="text-sm text-white/50">Check your email <span className="text-yellow-400 font-medium">{form.email}</span> to confirm your account.</p>
                <Link href="/login" className="inline-block text-sm text-yellow-400 hover:text-yellow-300 font-medium transition-colors">← Go to Login</Link>
              </div>
            ) : (
            <>
            <div className="flex flex-col gap-2">
              <label className="text-xs font-medium text-white/40">Full Name</label>
              <input type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Your full name" className="w-full rounded-xl px-4 py-3 text-sm text-white placeholder-white/20 outline-none focus:ring-2 focus:ring-yellow-400/30 transition-all" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }} />
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-xs font-medium text-white/40">Email</label>
              <input type="email" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="your@email.com" className="w-full rounded-xl px-4 py-3 text-sm text-white placeholder-white/20 outline-none focus:ring-2 focus:ring-yellow-400/30 transition-all" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }} />
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-xs font-medium text-white/40">Password</label>
              <input type="password" required value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="••••••••" className="w-full rounded-xl px-4 py-3 text-sm text-white placeholder-white/20 outline-none focus:ring-2 focus:ring-yellow-400/30 transition-all" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }} />
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-xs font-medium text-white/40">Confirm Password</label>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-3 rounded-xl text-sm font-medium text-white/60 hover:text-white transition-all" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
